...
function bindMapDrag(viewport, layer, points) {
  let start = null;
  let moved = 0;
  // Przejęcie wskaźnika przekierowuje pointerup na kadr, więc przy puszczeniu
  // palca nie da się już zapytać, co było pod spodem. Pinezkę zapamiętujemy
  // w momencie naciśnięcia — inaczej wizytówka nigdy by się nie otwierała.
  let downPin = null;
  ...
  viewport.addEventListener("pointerdown", (e) => {
    ...
    moved = 0;
    downPin = e.target.closest(".map-pin");
  ...
  const finish = (e) => {
    ...
    if (moved <= 8) {
      layer.style.transform = "";
      if (downPin) showMapCard(viewport, points, Number(downPin.dataset.index));
